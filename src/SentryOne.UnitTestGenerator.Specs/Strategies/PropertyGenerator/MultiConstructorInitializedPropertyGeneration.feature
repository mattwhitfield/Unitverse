...
namespace TestNamespace.SubNameSpace
{
	...
	Then I expect a method called 'ThisIsAPropertyIsInitializedCorrectly'
		...
		And I expect it to contain the statement '_testClass = new TestClass(_thisIsAProperty, _iTest);'
		And I expect it to contain 2 statements called 'Assert.That(_testClass.ThisIsAProperty, Is.EqualTo(_thisIsAProperty));'
	And I expect a method called 'StringPropIsInitializedCorrectly'
		And I expect it to contain the statement '_testClass = new TestClass(_stringProp);'
		And I expect it to contain 2 statements called 'Assert.That(_testClass.StringProp, Is.EqualTo(_stringProp));'
